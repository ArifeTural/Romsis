
export const data = [
  
    {
      id:1,
      image:"./video/giris.mp4",
    },
    {
     id:2,
     image:"./video/yazilimvideo.mp4",
    },
];

export const mobilgif =[
  {
    id:1,
    video:"/video/uygulamagiff.mp4"
  }
]



// export const yalinuretim=[ 
//   {
//     title: "Uçtan Uca Üretim Proses Analizi",
//     text: "Müşterilerimizin tüm üretim süreçlerini baştan sona analiz ediyoruz. Bu analiz, mevcut durumun değerlendirilmesi ve iyileştirme alanlarının belirlenmesidir.",
//     example: "Örnek: Bir otomotiv üreticisi için, montaj hattındaki her adımı gözden geçirerek zaman kayıplarını ve darboğazları belirliyoruz."
//   },
//   {
//     title: "Eksik Süreçlerin Tamamlanması",
//     text: "Eksik süreçleri tamamlayarak üretimdeki aksaklıkları gideriyoruz. Bu süreç, çalışanların eğitimini ve ekipman güncellemelerini içerir.",
//     example: "Örnek: Geri bildirimlerden yola çıkarak, ekipman kullanımında yaşanan sorunları çözüyoruz ve gerekli eğitimleri sağlıyoruz."
//   },
//   {
//     title: "Gereksiz Süreçlerin Kaldırılması",
//     text: "Gereksiz süreçleri kaldırarak maliyetleri azaltıyor ve daha verimli bir üretim sağlıyoruz. Bu, sadece zaman tasarrufu değil, aynı zamanda kaynakların daha etkili kullanılmasına yardımcı olur.",
//     example: "Örnek: Depolama süreçlerini optimize ederek, envanterdeki ürünlerin daha hızlı hareket etmesini sağlıyoruz."
//   },
//   {
//     title: "Verimlilik Artışı Sağlama",
//     text: "Verimliliği artırmak için sürekli iyileştirme çalışmalarını yürütüyoruz. Bu, süreçlerin analiz edilmesi, çalışanların motivasyonunu artıracak stratejiler geliştirilmesi ve teknoloji kullanımını içerir.",
//     example: "Örnek: Yapay zeka destekli süreç izleme sistemleri kurarak, anlık verimlilik oranlarını takip ediyoruz."
//   },
//   {
//     title: "İzlenebilir ve Yönetilebilir İş Akışları",
//     text: "İş akışlarını detaylandırarak izlenebilir ve yönetilebilir üretim süreçleri sunuyoruz. Bu, yöneticilerin süreçleri daha iyi yönetmesine ve performansı artırmasına olanak tanır.",
//     example: "Örnek: Üretim hatlarımızda gerçek zamanlı izleme sistemleri kurarak, her aşamanın verimliliğini anlık olarak raporluyoruz."
//   },
//   {
//     title: " Sürekli İyileştirme ve Güncelleme",
//     text: "Sürekli iyileştirme felsefesi ile, süreçlerimizi sürekli olarak gözden geçiriyoruz. Gözlemlerimizi değerlendiyoruz ve geliştirmeye yönelik adımlar atıyoruz. Bu adımları atarken sistemli bir yol izliyoruz.",
//     example: "Örnek: Takım üyeleriyle düzenli geri bildirim toplantıları yaparak, önerilen iyileştirmeleri hayata geçiriyoruz."


//   }
// ]


//   export const tarih =[
//     {
//       id:1,
//       title:"1997",
//       text:"Üretim sektöründe faaliyet gösteren firmaların WMS (Depo Yönetim Sistemi) süreçlerini daha pratik ve izlenebilir hale getirmek amacıyla proje geliştirmek için kurulduk.",
   
   
//      },
//     {
//       id:2,
//       title:"2001",
//       text:"Sermaye firmasına dönüşerek büyüme yolculuğumuza kararlılıkla devam ettik.",
   
   
//      },
//     {
//       id:3,
//       title:"2006",
//       text:"İlk yurt dışı projemizi başarıyla hayata geçirdik.",
   
   
//      },
//     {
//       id:4,
//       title:"2011",
//       text:"Havacılık sektörüne özel, uçak içi satış operasyonlarını ve back-office süreçlerini yönetebilecekleri bir ödeme yönetim sistemi projesini devreye aldık.",
   
   
//      },
//     {
//       id:5,
//       title:"2019",
//       text:"Uçak içi ikram satış projemiz olan Buy On Board’u başarıyla devreye aldık.",
   
   
//      },
//     {
//       id:6,
//       title:"2024",
//       text:"Türkiye’deki ve dünyadaki havayolu firmalarına ödeme çözümleri ve ikram yönetim sistemleri geliştirmeye devam ediyoruz.",
   
   
//      },


//   ]

//   export const ekip = [
//     {
//       id: 1,
//       image: "/image/ekip/esinaltin.jpeg",
//       title1: "Esin ALTIN",
//       title2: "Teknolojiden Sorumlu Genel Müdür Yardımcısı",
//       linkedin: "https://www.linkedin.com/in/esin-altin-9b182528/"
//     },
//     {
//       id: 2,
//       image: "/image/ekip/engin.jpeg",
//       title1: "Engin YILMAZ",
//       title2: "Operasyondan Sorumlu Genel Müdür Yardımcısı",
//       linkedin: "https://www.linkedin.com/in/romsis-yaz%C4%B1l%C4%B1m-581048115/"
//     },
//     {
//       id: 3,
//       image: "/image/ekip/tansu.jpeg",
//       title1: "Tansu ÇITLAK",
//       title2: "Operasyondan Sorumlu Direktör",
//       linkedin: "https://www.linkedin.com/in/tansu-%C3%A7itlak-131b1520b/"
//     },
//     {
//       id: 4,
//       image: "/image/ekip/st.jpeg",
//       title1: "Selim TURAL",
//       title2: "CTO",
//       linkedin: "https://www.linkedin.com/in/selim-tural-executive-mba-b34235161/"
//     },
//     {
//       id: 5,
//       image: "/image/ekip/abdurrahman.jpeg",
//       title1: "Abdurrahman ELIKARA",
//       title2: "Yazılım Takım Lideri",
//       linkedin: "https://www.linkedin.com/in/abdurrahman-e-91276b45/"
//     },
//     {
//       id: 6,
//       image: "/image/ekip/ayse.jpeg",
//       title1: "Ayşe Gül GÜÇLÜ",
//       title2: "Muhasebe Müdürü",
//       linkedin: "https://www.linkedin.com/in/romsis-yaz%C4%B1l%C4%B1m-581048115/"
//     }
//   ];  







export const yalinuretim = [
  {
    title: "t('yalinuretim.1.title')",
    text: "t('yalinuretim.1.text')",
    example: "t('yalinuretim.1.example')"
  },
  {
    title: "t('yalinuretim.2.title')",
    text: "t('yalinuretim.2.text')",
    example: "t('yalinuretim.2.example')"
  },
  {
    title: "t('yalinuretim.3.title')",
    text: "t('yalinuretim.3.text')",
    example: "t('yalinuretim.3.example')"
  },
  {
    title: "t('yalinuretim.4.title')",
    text: "t('yalinuretim.4.text')",
    example: "t('yalinuretim.4.example')"
  },
  {
    title: "t('yalinuretim.5.title')",
    text: "t('yalinuretim.5.text')",
    example: "t('yalinuretim.5.example')"
  },
  {
    title: "t('yalinuretim.6.title')",
    text: "t('yalinuretim.6.text')",
    example: "t('yalinuretim.6.example')"
  }
];

export const tarih = [
  {
    id: 1,
    title: "t('tarih.1.title')",
    text: "t('tarih.1.text')"
  },
  {
    id: 2,
    title: "t('tarih.2.title')",
    text: "t('tarih.2.text')"
  },
  {
    id: 3,
    title: "t('tarih.3.title')",
    text: "t('tarih.3.text')"
  },
  {
    id: 4,
    title: "t('tarih.4.title')",
    text: "t('tarih.4.text')"
  },
  {
    id: 5,
    title: "t('tarih.5.title')",
    text: "t('tarih.5.text')"
  },
  {
    id: 6,
    title: "t('tarih.6.title')",
    text: "t('tarih.6.text')"
  }
];

export const ekip = [
  {
    id: 1,
    image: "/image/ekip/esinaltin.jpeg",
    title1: "t('ekip.1.title1')",
    title2: "t('ekip.1.title2')",
    linkedin: "https://www.linkedin.com/in/esin-altin-9b182528/"
  },
  {
    id: 2,
    image: "/image/ekip/engin.jpeg",
    title1: "t('ekip.2.title1')",
    title2: "t('ekip.2.title2')",
    linkedin: "https://www.linkedin.com/in/romsis-yaz%C4%B1l%C4%B1m-581048115/"
  },
  {
    id: 3,
    image: "/image/ekip/tansu.jpeg",
    title1: "t('ekip.3.title1')",
    title2: "t('ekip.3.title2')",
    linkedin: "https://www.linkedin.com/in/tansu-%C3%A7itlak-131b1520b/"
  },
  {
    id: 4,
    image: "/image/ekip/st.jpeg",
    title1: "t('ekip.4.title1')",
    title2: "t('ekip.4.title2')",
    linkedin: "https://www.linkedin.com/in/selim-tural-executive-mba-b34235161/"
  },
  {
    id: 5,
    image: "/image/ekip/abdurrahman.jpeg",
    title1: "t('ekip.5.title1')",
    title2: "t('ekip.5.title2')",
    linkedin: "https://www.linkedin.com/in/abdurrahman-e-91276b45/"
  },
  {
    id: 6,
    image: "/image/ekip/ayse.jpeg",
    title1: "t('ekip.6.title1')",
    title2: "t('ekip.6.title2')",
    linkedin: "https://www.linkedin.com/in/romsis-yaz%C4%B1l%C4%B1m-581048115/"
  }
];
